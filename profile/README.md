[![Logo Image](https://raw.githubusercontent.com/tendixhq/.github/main/tendix.png)](https://github.com/VicinityTeam/talon)

# ⚡ Tendix

**We’re building the EEA’s best Point of Sale system.**

Tendix is a modern, enterprise-grade POS platform designed to **supercharge retail businesses** — from independent shops to multi-store operations.

Fast. Reliable. Built for real-world retail.

---

## 🚀 What We’re Building

- 🏪 **Retail-first POS** (desktop & web)
- 🔌 **Offline-first architecture** — sell without internet
- 📦 **Accurate inventory & stock movements**
- 📊 **Clear reporting & end-of-day summaries**
- 🔐 **Secure, scalable, EEA-ready**

---

## 🌍 Our Vision

Retail software should be:
- Fast
- Reliable
- Simple to use
- Powerful under the hood

Tendix aims to become the **default POS platform across the EEA**.

---

⚡ **Supercharge your retail business with Tendix.**
